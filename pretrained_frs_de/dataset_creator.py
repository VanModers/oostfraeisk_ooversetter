...
class TranslationDataset(Dataset):
    def __init__(self, path, tokenizer, max_length=512):
        ...
        with open(os.path.join(path, "german.txt"), "r", encoding="utf-8") as ger_file, \
            ...
            for ger, frs in zip(ger_file, frs_file):
                self.examples.append(frs.strip())
                self.labels.append(ger.strip())
    ...
    def __getitem__(self, idx):
        ...
        input_encodings = self.tokenizer(example, truncation=True, padding='max_length', max_length=self.max_length, return_tensors="pt")
        
        # Tokenize the label (East Frisian)
        label_encodings = self.tokenizer(label, truncation=True, padding='max_length', max_length=self.max_length, return_tensors="pt")
